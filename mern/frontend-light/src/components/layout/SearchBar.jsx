...
const SearchBar = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      navigate(`/shop?search=${encodeURIComponent(searchTerm)}&page=1&limit=9&sort=featured`);
      setSearchResults([]);
      setSearchTerm('');
    }
  };
  ...
  return (
    <div ref={searchRef} className="relative hidden md:block">
      <form onSubmit={handleSubmit} className="flex items-center">
        <div className="flex items-center w-64 bg-white border border-gray-200 rounded-full overflow-hidden">
          <button
            type="submit"
